...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("DONMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        if (pblock->IsZerocoinStake()) {
            ...
            pwalletMain->zdonTracker->ListMints(true, true, true); //update the state
        }
        return error("DONMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("DONMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("don-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running DONMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
