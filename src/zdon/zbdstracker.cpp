...
CoinWitnessData* CzDONTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzDONTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
void CzDONTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzDONWallet* zDONWallet)
{
    bool iszDONWalletInitialized = (NULL != zDONWallet);
    CMintMeta meta;
    ...
    if (! iszDONWalletInitialized)
        zDONWallet = new CzDONWallet(strWalletFile);
    meta.isSeedCorrect = zDONWallet->CheckSeed(dMint);
    if (! iszDONWalletInitialized)
        delete zDONWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
std::set<CMintMeta> CzDONTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzDONWallet* zDONWallet = new CzDONWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zDONWallet);
        }
        delete zDONWallet;
