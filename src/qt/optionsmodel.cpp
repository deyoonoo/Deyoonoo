...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDONAmount"))
        settings.setValue("nAnonymizeDONAmount", 1000);

    nAnonymizeDONAmount = settings.value("nAnonymizeDONAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDONAmount"))
        SoftSetArg("-anonymizedonamount", settings.value("nAnonymizeDONAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in don.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDONAmount:
            return QVariant(nAnonymizeDONAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeDONAmount:
            nAnonymizeDONAmount = value.toInt();
            settings.setValue("nAnonymizeDONAmount", nAnonymizeDONAmount);
            emit anonymizeDONAmountChanged(nAnonymizeDONAmount);
            break;
        case CoinControlFeatures:
