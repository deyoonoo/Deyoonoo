...
void ZDONControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zdonTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zdonTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zDON.";
            else if (!mint.isSeedCorrect)
                strReason = "The zDON seed used to mint this zDON is not the same as currently hold in the wallet";
            else
