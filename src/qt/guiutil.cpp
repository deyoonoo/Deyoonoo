...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a DON address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no DON: URI
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert don:// to don:
    //
    //    Cannot handle this later, because don:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open don.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DON.lnk";
}
...
{
    return GetAutostartDir() / "don.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a don.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the don app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add don app to startup item list
